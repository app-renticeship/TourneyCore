    ...
    public class PointsDB
    {
        internal PointsDB()
        {
            ...
            catch (MySqlException ex)
            {
                if (ex.Number == 1042)
                {
                    Logger.LogError("[TourneyCore] Can't connect to MySQL host.");
                }
                else
                {
        ...
        private MySqlConnection CreateConnection()
        {
            ...
            try
            {
                if (Init.Instance.Configuration.Instance.PDatabasePort == 0) Init.Instance.Configuration.Instance.PDatabasePort = 3306;
                connection = new MySqlConnection(String.Format("SERVER={0};DATABASE={1};UID={2};PASSWORD={3};PORT={4};", Init.Instance.Configuration.Instance.PDatabaseAddress, Init.Instance.Configuration.Instance.PDatabaseName, Init.Instance.Configuration.Instance.PDatabaseUsername, Init.Instance.Configuration.Instance.PDatabasePassword, Init.Instance.Configuration.Instance.PDatabasePort));
            }
            catch (Exception ex)
            {
                Logger.LogException(ex);
        ...
        private void AddPointsThread(object objectAddSetPointsParameter)
        {
            try
            {
                ...
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "UPDATE `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` SET `points`=`points`+" + classAddSetPointsParameter.points.ToString() + " WHERE `steamId`=@steamId;";
        ...
        public string[] GetAccountBySteamID(string steamId)
        {
            ...
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = new MySqlCommand("SELECT * FROM (SELECT t.steamId, t.points, t.lastDisplayName, @`rownum` := @`rownum` + 1 AS currentRank FROM `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` t JOIN (SELECT @`rownum` := 0) r ORDER BY t.points DESC) x WHERE x.steamId = @steamId;", connection);
        ...
        public string[] GetAccountByRank(int rank)
        {
            ...
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = new MySqlCommand("SELECT * FROM (SELECT t.steamId, t.points, t.lastDisplayName, @`rownum` := @`rownum` + 1 AS currentRank FROM `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` t JOIN (SELECT @`rownum` := 0) r ORDER BY t.points DESC) x WHERE x.currentRank = " + rank.ToString() + ";", connection);
                connection.Open();
                MySqlDataReader dataReader = command.ExecuteReader(System.Data.CommandBehavior.SingleRow);
                while (dataReader.Read())
                {
        ...
        public int GetRankBySteamID(string steamId)
        {
            ...
            try
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT `currentRank` FROM (SELECT t.steamId, t.points, @`rownum` := @`rownum` + 1 AS currentRank FROM `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` t JOIN (SELECT @`rownum` := 0) r ORDER BY t.points DESC) x WHERE x.steamId = @steamId;";
                command.Parameters.AddWithValue("@steamId", steamId);
                connection.Open();
        ...
        public string[] GetTopRanks(int limit)
        {
            ...
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = new MySqlCommand("SELECT * FROM (SELECT t.steamId, t.points, t.lastDisplayName, @`rownum` := @`rownum` + 1 AS currentRank FROM `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` t JOIN (SELECT @`rownum` := 0) r ORDER BY t.points DESC) x LIMIT 0," + limit.ToString() + ";", connection);
                connection.Open();
        ...
        internal void CreateCheckSchema()
        {
            try
            {
                ...
                MySqlCommand command = connection.CreateCommand();
                connection.Open();
                command.CommandText = "SHOW TABLES LIKE '" + Init.Instance.Configuration.Instance.PDatabaseTableName + "';";
                object test = command.ExecuteScalar();
                if (test == null)
                {
                    command.CommandText = "CREATE TABLE `" + Init.Instance.Configuration.Instance.PDatabaseTableName + "` (`steamId` VARCHAR(32) NOT NULL, `points` INT(32) NOT NULL DEFAULT '0', `lastDisplayName` varchar(32) NOT NULL, `lastUpdated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (`steamId`));";
                    command.ExecuteNonQuery();
                }
                connection.Close();
            }
